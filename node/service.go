...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (cmbt.Database, error) {
	if ctx.config.DataDir == "" {
		return cmbt.NewMemDatabase()
	}
	db, err := cmbt.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
