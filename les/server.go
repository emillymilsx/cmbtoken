...
// Package les implements the Light cmbtereum Subprotocol.
package les
...
import (
	...
	"github.com/CoinMarketBrasil/cmbtoken/cmbt"
	"github.com/CoinMarketBrasil/cmbtoken/cmbtdb"
	"github.com/CoinMarketBrasil/cmbtoken/les/flowcontrol"
	"github.com/CoinMarketBrasil/cmbtoken/light"
	"github.com/CoinMarketBrasil/cmbtoken/log"
	"github.com/CoinMarketBrasil/cmbtoken/p2p"
	"github.com/CoinMarketBrasil/cmbtoken/p2p/discv5"
...
func NewLesServer(cmbt *cmbt.cmbtereum, config *cmbt.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(cmbt.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, cmbt.EventMux(), cmbt.Engine(), newPeerSet(), cmbt.BlockChain(), cmbt.TxPool(), cmbt.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range ServerProtocolVersions {
		lesTopics[i] = lesTopic(cmbt.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(cmbt.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(cmbt.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(cmbt.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(cmbt.ChainDb())
	return srv, nil
}
...
func newCostStats(db cmbtdb.Database) *requestCostStats {
