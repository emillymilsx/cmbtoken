...
//
// The cmbtoken library is distributed in the hope that it will be useful,
...
import (
	...
	"github.com/CoinMarketBrasil/cmbtoken/common/bitutil"
...
func GetChtRoot(db cmbt.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db cmbt.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db cmbt.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db cmbt.Database, clientMode bool) *core.ChainIndexer {
	cdb := cmbt.NewTable(db, ChtTablePrefix)
	idb := cmbt.NewTable(db, "chtIndex-")
...
func GetBloomTrieRoot(db cmbt.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db cmbt.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db cmbt.Database, clientMode bool) *core.ChainIndexer {
	cdb := cmbt.NewTable(db, BloomTrieTablePrefix)
	idb := cmbt.NewTable(db, "bltIndex-")
