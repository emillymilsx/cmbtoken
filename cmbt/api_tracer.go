...
import (
	...
	"github.com/CoinMarketBrasil/cmbtoken/cmbt/tracers"
	"github.com/CoinMarketBrasil/cmbtoken/cmbtdb"
	"github.com/CoinMarketBrasil/cmbtoken/internal/cmbtapi"
	"github.com/CoinMarketBrasil/cmbtoken/log"
	"github.com/CoinMarketBrasil/cmbtoken/rlp"
	"github.com/CoinMarketBrasil/cmbtoken/rpc"
	"github.com/CoinMarketBrasil/cmbtoken/trie"
)
...
type ephemeralDatabase struct {
	diskdb cmbtdb.Database     // Persistent disk database to fall back to with reads
	memdb  *cmbtdb.MemDatabase // Ephemeral memory database for primary reads and writes
}
...
func (db *ephemeralDatabase) NewBatch() cmbtdb.Batch {
...
func (db *ephemeralDatabase) Prune(root common.Hash) {
	...
	db.memdb, _ = cmbtdb.NewMemDatabaseWithCap(db.memdb.Len())
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.cmbt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.cmbt.blockchain.CurrentBlock()
	default:
		from = api.cmbt.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.cmbt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.cmbt.blockchain.CurrentBlock()
	default:
		to = api.cmbt.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	memdb, _ := cmbtdb.NewMemDatabase()
	db := &ephemeralDatabase{
		diskdb: api.cmbt.ChainDb(),
		memdb:  memdb,
	}
	if number := start.NumberU64(); number > 0 {
		start = api.cmbt.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.cmbt.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.cmbt.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.cmbt.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.cmbt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.cmbt.blockchain.CurrentBlock()
	default:
		block = api.cmbt.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.cmbt.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.cmbt.engine.VerifyHeader(api.cmbt.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.cmbt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.cmbt.blockchain, nil)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.cmbt.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.cmbt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.cmbt.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := core.GetTransaction(api.cmbt.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &cmbtapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  cmbtapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.cmbt.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.cmbt.blockchain, nil)
		if idx == txIndex {
